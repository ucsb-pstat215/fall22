...
a) Implement the hierarchical binomial model following the example on the model for rat tumors in Section 5.3 (or the basketball example from class).  Use the noninformative hyperprior for $a$ and $b$ (see textbook) or choose your own proper subjective prior on $a/(a+b)$ and $log(a+b)$ based on your knowledge about baseball (note you'll need to include the jacobian for the prior density (a+b)^{-5/2}.  Why?).  
    - Use grid sampling to draw samples from the marginal posterior $p(\alpha, \beta \mid y)$.  Use `optim` to find the mode of the marginal posterior and adjust the limits of your grid by trial and error.
